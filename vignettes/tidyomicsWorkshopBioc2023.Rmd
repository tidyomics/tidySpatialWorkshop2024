...
g$tile <- rep(1:5, c(100,100,100,100,47))
...
sce_sub |>
  join_features(features = head(gene_names)) |>
  left_join(g_mcols, by = join_by(.feature == symbol)) |>
  select(.cell, .feature, tile)
...
`tidyr` and `purrr` offer a great tool to perform iterativre analyses
in a functional way.
