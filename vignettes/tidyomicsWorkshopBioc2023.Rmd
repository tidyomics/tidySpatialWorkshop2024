...
# Part 2b Genomic and transcriptomic integration
...
we will add genomic location information, and operate on this
information using the *plyranges* package. This package provides tidy
functionality to genomic ranges (*GRanges* objects) analogously to how
...
```{r}
# this could be done with plyranges instead...
rowRanges(sce_sub) <- rowRanges(sce_sub) |>
  mutate(tile = rep(1:5, c(100,100,100,100,47)))

# clunky
rr <- rowRanges(sce_sub) |>
  select(symbol, tile, .drop_ranges=TRUE) |>
  as_tibble() |>
...
nested <- sce_sub |>
  ...
  left_join(rr, by="feature") |>
  nest(se = -tile)

smry <- nested |>
  mutate(summary = map(se, \(se) {
    tibble(count_sum = colSums(assay(se)),                             
           cell_type = se$cell_type)
  })
  ) |>
  select(-se) |>
  unnest(summary) |>
  mutate(ct = substr(cell_type, 1, 8))

library(forcats)
smry |>
  mutate(ct = fct_reorder(ct, count_sum, .fun=median)) |>
  ggplot(aes(count_sum, ct)) +
  geom_point() +
  facet_wrap(~tile, labeller=label_both)
```

# Part 3 Nested analyses

When analysing single cell data is sometimes necessary to perform
...
`tidyr` and `purrr` offer a great tool to perform iterative analyses
in a functional way.

We use tidyverse `nest` to group the data. The command below will
create a tibble containing a column with a SummarizedExperiment object
for each cell type. `nest` is similar to tidyverse `group_by`, except
with `nest` each group is stored in a single row, and can be a complex
object such as `SingleCellExperiment`. 
...
Now, let's perform a differential gene-transcript abundance analysis
between the two conditions for each cell type. 
...
We can the lies the top genes with the heat map iteratively across the
cell types
...
1. Let's suppose that you want to perform the analyses only for cell
   types that have a total number of cells bigger than 1000. For
   example, if a cell type has less than a sum of 1000 cells across
   all samples, that cell type will be dropped from the dataset. 

- Answer this question avoiding to save temporary variables, and using
