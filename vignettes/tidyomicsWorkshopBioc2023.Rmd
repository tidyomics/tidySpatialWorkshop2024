...
# Part 3 Genomic and transcriptomic integration
...
package, and tie this to the transcriptomics data we have been examining.

*plyranges* provides tidyverse-style functionality to genomic ranges
(*GRanges* objects) analogously to how *tidySingleCellExperiment*
...
the object. That is, we would have information about the start and end
of each gene, their strand, the lengths of the chromosomes, the genome
build, etc. Here we don't have range information yet, so we will first
take some steps to add hg38 range information on our genes, matching
by the gene symbol provided on the rows of `sce_obj`.
...
# recommend Ensembl or GENCODE gene annotations
...
column, using the `select()` function.
...
Now we filter the genes to only those present as rownames of
`sce_obj`, and remove duplicate IDs. We then sort by genomic location,
and use the gene symbols are names of the ranges.
...
```{r}
sce_sub <- sce_obj[ names(g), ]
rowRanges(sce_sub) <- g
sce_sub |> rowRanges() # peek at the ranges
```

Now let's do something interesting with the gene ranges: let's see if
genes near peaks of active chromatin marks (H3K3me3) in PBMC have a
difference in their expression level. There are many sources of
epigenetic tracks, ENCODE, Roadmap, etc. Here we will use some ENCODE
data available in *AnnotationHub* on Bioconductor. 

We have to do a bit of book-keeping first. These peaks were in
the hg19 genome build, so we have ran the following commands (here
un-evaluated) to lift the peaks to hg38, and provide proper sequence
information.
...
Loading those previously generated peaks:
...
dists <- dists |>
  ...
  select(symbol, distance, .drop_ranges = TRUE) |>
  as_tibble() |>
